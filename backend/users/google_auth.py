...
def authenticate_google_credential(credential):
    ...
    return (
        email,
        (token_info.get("given_name") or "").strip(),
        (token_info.get("family_name") or "").strip(),
    )


def get_or_create_google_user(email, first_name="", last_name=""):
    """Return the BudgetBuddy user owned by this verified Google email.

    Email is the account identity. Existing accounts are always reused; a new
    account is created only when no BudgetBuddy user owns the email. The
    database unique constraint is the final protection against concurrent
    Google-login requests creating duplicate accounts.
    """
    ...
    try:
        with transaction.atomic():
            # Keep username generation exactly as before, but perform creation
            # inside the transaction so the email uniqueness constraint can
            # safely resolve concurrent requests.
        ...
        # Another request may have claimed this email after our initial lookup.
        # Reuse that account instead of surfacing a duplicate-account error.
        user = User.objects.filter(email__iexact=email).first()
        if user is None:
            raise
