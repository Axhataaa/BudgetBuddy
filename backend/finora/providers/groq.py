...
# NOTE: these prompts are copied verbatim from finora/providers/gemini.py.
# They are vendor-agnostic instructions to the model, not Gemini-specific
# API shaping, so they carry over unchanged for Groq.
...
def _call_groq(*, api_key, model, system_prompt, messages, json_mode=False):
    ...
    try:
        ...
        logger.error("Finora: Groq authentication error - %s", exc)
        raise FinoraProviderUnavailable("auth_error") from exc
    except groq.RateLimitError as exc:
        logger.error("Finora: Groq rate limit hit - %s", exc)
        raise FinoraProviderUnavailable("rate_limited") from exc
    except groq.APITimeoutError as exc:
        logger.error("Finora: Groq request timed out - %s", exc)
        raise FinoraProviderUnavailable("timeout") from exc
    except groq.APIConnectionError as exc:
        logger.error("Finora: Groq network error - %s", exc)
        raise FinoraProviderUnavailable("network_error") from exc
    except groq.APIStatusError as exc:
        logger.error("Finora: Groq HTTP %s - %s", exc.status_code, exc)
        raise FinoraProviderUnavailable("http_error") from exc
    except groq.GroqError as exc:
        logger.error("Finora: Groq SDK error - %s", exc)
        raise FinoraProviderUnavailable("provider_error") from exc

    try:
        text = completion.choices[0].message.content
    except (AttributeError, IndexError, TypeError) as exc:
        logger.error("Finora: unexpected Groq response shape - %s", exc)
...
class GroqProvider(FinoraAIProvider):
    ...
    def _get_credentials(self):
        ...
        if not api_key:
            logger.error(
                "Finora: GROQ_API_KEY is not configured. Set it in the "
    ...
    def generate_reply(self, *, context, message, history, scenario_result=None):
        ...
        messages = [
            ...
            {
                "role": "assistant",
                "content": "Understood. I'll use only that data to help.",
            },
        ]
        ...
        return _call_groq(
            api_key=api_key, model=model, system_prompt=SYSTEM_PROMPT, messages=messages
        )
